...
print('Erro na definição do DataFrame')
...
print('Erro em mandar Jogos para o Banco')
# Inserindo dados na tabela 'Jogos' para obter os IDs
jogos_df.to_sql('Jogos', engine, if_exists='replace', index=False)

print('Erro na consulta de ID_Game e Games em Jogos')
# Mapeando o ID_Game para a tabela Eventos
try:
    sql_query = """SELECT ID_Game, Games FROM Jogos"""
    jogos_ids = session.execute(text(sql_query))
except Exception as e:
    print(f"Erro na consulta de ID_Game em tabela Jogos: {e}")
# jogos_ids = pd.read_sql('SELECT ID_Game, Games FROM Jogos', engine)

print('Erro na definição do DataFrame eventos')
...
print('Erro em mandar Jogos para o Atletas, Paises, eventos')
